...
new #[Title('Edit Promo')] class extends Component {
    public Promo $promo;
    ...
    public function mount(Promo $promo): void
    {
        ...
        foreach ($promo->produkVarians()->with('produk')->get() as $varian) {
            $this->selectedVarians[] = [
                ...
                'harga_modal' => (string) $varian->harga_modal,
                'minimal_harga_jual' => (string) $varian->pivot->minimal_harga_jual,
            ];
        }
    }
    ...
    public function save(): void
    {
        ...
        foreach ($validated['selectedVarians'] as $varian) {
            $syncData[$varian['produk_varian_id']] = [
                'minimal_harga_jual' => $varian['minimal_harga_jual'],
            ];
        }
        $this->promo->produkVarians()->sync($syncData);

        Flux::toast(variant: 'success', text: __('Promo berhasil diperbarui.'));
...
<div class="mx-auto max-w-2xl space-y-6">
    ...
    <div class="flex items-center gap-3">
        ...
        <div>
            <flux:heading size="xl">{{ __('Edit Promo') }}</flux:heading>
            <flux:subheading>{{ $promo->nama }}</flux:subheading>
        </div>
    </div>
